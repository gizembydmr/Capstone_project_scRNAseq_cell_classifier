...
"""
Compare Logistic Regression, Linear SVM, and Random Forest models for
Tabula Sapiens pancreas Level 3 cell-type prediction.

Each model is tested with and without class_weight='balanced'.
The script uses an 80/20 stratified development-test split, then runs
5-fold stratified CV on the development set.
"""
...
DATA_PATH = Path(
    r"C:\Users\ferid\Downloads\capstone_demo\pancreas_model\preprocess_train_pancreas_outputs\Tabula_Sapiens_Pancreas_preprocessed_for_training.h5ad"
)
...
RESULTS_DIR = PROJECT_DIR / "results" / "all_models_level3"
FIGURES_DIR = RESULTS_DIR / "figures"

RESULTS_DIR.mkdir(parents=True, exist_ok=True)
FIGURES_DIR.mkdir(parents=True, exist_ok=True)

EXCEL_PATH = RESULTS_DIR / "pancreas_all_models_level3_CV_results.xlsx"
METADATA_PATH = RESULTS_DIR / "pancreas_all_models_level3_metadata.json"
SPLIT_PATH = RESULTS_DIR / "pancreas_all_models_level3_split_indices.npz"
...
def save_presentation_summary_table(summary_df, output_path):
    ...
    table.scale(1, 1.45)
    ax.set_title("Pancreas Level 3 Model Comparison: 5-Fold CV Summary", pad=12)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()
...
if class_counts.min() < N_SPLITS:
    raise ValueError(
        f"At least one class has fewer than {N_SPLITS} cells. "
        "5-fold stratified CV is not valid for this label level."
    )


# Split once and reuse this file later
...
if len(set(dev_indices).intersection(set(test_indices))) > 0:
    raise ValueError("Development and test indices overlap. Please check the split.")
...
if dev_counts_check.min() < N_SPLITS:
    raise ValueError(
        ...
        "5-fold stratified CV is not valid for this split."
    )
...
print("\nTraining each model on the full development set and testing once...")
...
experiment_info = pd.DataFrame({
    ...
    "value": [
        ...
        N_DEV,
        N_TEST,
        "80/20 stratified train_test_split",
...
metadata = {
    "experiment_name": "pancreas_all_models_level3_CV",
    ...
    "split_type": "80/20 stratified train_test_split",
...
save_presentation_summary_table(
    ...
    output_path=FIGURES_DIR / "pancreas_all_models_level3_presentation_summary_table.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="f1_macro_mean",
    title="Pancreas Level 3 Model Comparison: Macro F1 Mean Across 5-Fold CV",
    output_path=FIGURES_DIR / "pancreas_all_models_level3_macro_f1_barplot.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="f1_weighted_mean",
    title="Pancreas Level 3 Model Comparison: Weighted F1 Mean Across 5-Fold CV",
    output_path=FIGURES_DIR / "pancreas_all_models_level3_weighted_f1_barplot.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="balanced_accuracy_mean",
    title="Pancreas Level 3 Model Comparison: Balanced Accuracy Mean Across 5-Fold CV",
    output_path=FIGURES_DIR / "pancreas_all_models_level3_balanced_accuracy_barplot.png",
)

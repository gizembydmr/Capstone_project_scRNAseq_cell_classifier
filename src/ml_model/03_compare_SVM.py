...
"""
Compare Linear SVM models for PBMC68k cell type prediction.

This script compares:
1. SVM_no_weight
2. SVM_balanced

using stratified 5-fold cross-validation on the same 80% development set
created in the Logistic Regression experiment.
...
Important:
LinearSVC does not provide predict_proba(), so probability-based metrics
such as log loss and ROC AUC are saved as NaN.
"""
...
# Load the same split created by the Logistic Regression script
LR_SPLIT_PATH = (
    PROJECT_DIR
    / "results"
    / "LR_level1"
...
RESULTS_DIR = PROJECT_DIR / "results" / "SVM_level1"
FIGURES_DIR = RESULTS_DIR / "figures"

RESULTS_DIR.mkdir(parents=True, exist_ok=True)
FIGURES_DIR.mkdir(parents=True, exist_ok=True)

EXCEL_PATH = RESULTS_DIR / "SVM_level1_CV_results.xlsx"
METADATA_PATH = RESULTS_DIR / "SVM_level1_metadata.json"
...
def make_svm_model(class_weight):
    ...
    Notes
    -----
    LinearSVC is used instead of SVC(kernel='linear') because it is more suitable
    for larger high-dimensional datasets.
    """
    return LinearSVC(
        penalty="l2",
        loss="squared_hinge",
...
def calculate_metrics(y_true, y_pred):
    ...
    Since LinearSVC does not provide probability estimates by default,
    probability-based metrics are stored as NaN for consistency with
    LR/RF result tables.
    """
    metrics = {
        ...
        # Probability-based metrics are not available for LinearSVC
        "log_loss": np.nan,
...
def save_confusion_matrix_table(cm_df, title, output_path):
    """
    Save a confusion matrix as a readable table image.
    """
    table_df = cm_df.copy()
...
def save_summary_table_image(summary_df, output_path):
    ...
    ax.set_title("Linear SVM Level 1 CV Summary", pad=12)
...
# ============================================================
# 5. Load same development-test split from LR experiment
# ============================================================

if not LR_SPLIT_PATH.exists():
    raise FileNotFoundError(
        f"Could not find split index file:\n{LR_SPLIT_PATH}\n\n"
        "Please run 01_compare_LR.py first to create the split file."
    )

split_data = np.load(LR_SPLIT_PATH)
...
print(f"\nLoaded same split from: {LR_SPLIT_PATH}")
...
print("\nStarting 5-fold stratified cross-validation for Linear SVM...")
...
for model_name, model_info in models.items():
    ...
    for fold_number, (train_idx, val_idx) in enumerate(cv.split(X_dev, y_dev), start=1):
        ...
        print(
            f"    Fold {fold_number} training finished in "
...
SVM_no_weight_report = average_class_reports[
    average_class_reports["model_name"] == "SVM_no_weight"
].reset_index(drop=True)

SVM_balanced_report = average_class_reports[
    average_class_reports["model_name"] == "SVM_balanced"
].reset_index(drop=True)

SVM_no_weight_confusion_normalized = normalize_confusion_matrix_rows(
    confusion_matrices["SVM_no_weight"]
)

SVM_balanced_confusion_normalized = normalize_confusion_matrix_rows(
    confusion_matrices["SVM_balanced"]
)
...
experiment_info = pd.DataFrame({
    "field": [
        ...
        "probability_based_metrics_note",
    ],
    "value": [
        "SVM_level1_CV",
        ...
        ", ".join(models.keys()),
        "f1_macro_mean",
        "LinearSVC does not provide predict_proba; log_loss and ROC AUC are saved as NaN.",
    ],
})
...
metadata = {
    ...
    "split_source": str(LR_SPLIT_PATH),
    ...
    "probability_based_metrics_note": (
        ...
        "log_loss and ROC AUC are saved as NaN."
    ),
}
...
with pd.ExcelWriter(EXCEL_PATH, engine="openpyxl") as writer:
    ...
    confusion_matrices["SVM_no_weight"].to_excel(
        writer,
        sheet_name="SVM_no_weight_confusion",
    )

    confusion_matrices["SVM_balanced"].to_excel(
        writer,
        sheet_name="SVM_balanced_confusion",
    )

    SVM_no_weight_confusion_normalized.to_excel(
        writer,
        sheet_name="SVM_no_weight_conf_norm",
    )

    SVM_balanced_confusion_normalized.to_excel(
        writer,
        sheet_name="SVM_balanced_conf_norm",
    )

    SVM_no_weight_report.to_excel(
        writer,
        sheet_name="SVM_no_weight_report",
        index=False,
    )

    SVM_balanced_report.to_excel(
        writer,
        sheet_name="SVM_balanced_report",
        index=False,
    )
...
# ============================================================
# 12. Save PowerPoint-friendly figures
# ============================================================

save_summary_table_image(
    summary_df=summary_metrics,
    output_path=FIGURES_DIR / "SVM_level1_summary_table.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="f1_macro_mean",
    title="Linear SVM Level 1: Macro F1 Comparison",
    output_path=FIGURES_DIR / "SVM_level1_macro_f1_barplot.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="balanced_accuracy_mean",
    title="Linear SVM Level 1: Balanced Accuracy Comparison",
    output_path=FIGURES_DIR / "SVM_level1_balanced_accuracy_barplot.png",
)

save_confusion_matrix_table(
    cm_df=confusion_matrices["SVM_no_weight"],
    title="SVM no weight - summed CV confusion matrix",
    output_path=FIGURES_DIR / "SVM_level1_no_weight_confusion_table.png",
)

save_confusion_matrix_table(
    cm_df=confusion_matrices["SVM_balanced"],
    title="SVM balanced - summed CV confusion matrix",
    output_path=FIGURES_DIR / "SVM_level1_balanced_confusion_table.png",
)

save_normalized_confusion_matrix_table(
    cm_df=confusion_matrices["SVM_no_weight"],
    title="SVM no weight - normalized summed CV confusion matrix (%)",
    output_path=FIGURES_DIR / "SVM_level1_no_weight_confusion_table_normalized.png",
)

save_normalized_confusion_matrix_table(
    cm_df=confusion_matrices["SVM_balanced"],
    title="SVM balanced - normalized summed CV confusion matrix (%)",
    output_path=FIGURES_DIR / "SVM_level1_balanced_confusion_table_normalized.png",
)
...
print("\nLinear SVM cross-validation finished successfully.")
...
print("Note: LinearSVC does not provide predict_proba, so log_loss and ROC AUC are NaN.")
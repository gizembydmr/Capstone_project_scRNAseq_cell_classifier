...
using the fine hierarchical label level: cell_type_level_3.
...
EXCEL_PATH = RESULTS_DIR / "all_models_level3_CV_results.xlsx"
METADATA_PATH = RESULTS_DIR / "all_models_level3_metadata.json"
SPLIT_PATH = RESULTS_DIR / "all_models_level3_split_indices.npz"

# Reuse the previous Level 1 LR split to keep Level 1 and Level 3 comparisons consistent.
PREVIOUS_SPLIT_PATH = (
    PROJECT_DIR
    / "results"
    / "LR_level1"
...
def save_presentation_summary_table(summary_df, output_path):
    ...
    table.scale(1, 1.45)
    ax.set_title("Level 3 Model Comparison: 5-Fold CV Summary", pad=12)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()
...
print(f"Saved Level 3 split indices to: {SPLIT_PATH}")
print(f"Development set size: {N_DEV}")
print(f"Test set size: {N_TEST}")

# Warn if reused Level 1 split is not sufficiently stratified for Level 3 classes.
...
metadata = {
    "experiment_name": "all_models_level3_CV",
...
save_presentation_summary_table(
    ...
    output_path=FIGURES_DIR / "all_models_level3_presentation_summary_table.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="f1_macro_mean",
    title="Level 3 Model Comparison: Macro F1 Mean Across 5-Fold CV",
    output_path=FIGURES_DIR / "all_models_level3_macro_f1_barplot.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="f1_weighted_mean",
    title="Level 3 Model Comparison: Weighted F1 Mean Across 5-Fold CV",
    output_path=FIGURES_DIR / "all_models_level3_weighted_f1_barplot.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="balanced_accuracy_mean",
    title="Level 3 Model Comparison: Balanced Accuracy Mean Across 5-Fold CV",
    output_path=FIGURES_DIR / "all_models_level3_balanced_accuracy_barplot.png",
)

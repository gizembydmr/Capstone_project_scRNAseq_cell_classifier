# 08_compare_all_models_level3_PCA.py

"""
Compare three machine learning model families for PBMC68k cell type prediction
using the fine hierarchical label level: cell_type_level_3, with PCA-style
dimensionality reduction added inside the model pipeline.

This script is designed as a PCA-version companion to:
07_compare_all_models_level3.py

It compares 6 configurations:
...
Main update compared with the previous Level 3 script:
- The input is still the 2000-HVG matrix from pbmc68k_preprocessed_for_training.h5ad.
- A dimensionality reduction step is added before each classifier.
- The dimensionality reduction step is fitted only on the training part of each CV fold.
- The validation fold is transformed using the dimensionality reduction fitted on the
  corresponding training fold. This avoids PCA-related data leakage.

Memory note:
- Because a laptop with 8GB RAM may struggle with dense PCA on ~67k cells x 2000 genes,
  this script uses TruncatedSVD by default.
- TruncatedSVD is sparse-friendly and commonly used as a PCA-like dimensionality
  reduction method for sparse high-dimensional matrices.
- If you explicitly want real centered PCA, you can set DIM_REDUCTION_METHOD = "pca_dense",
  but this may require more RAM.
...
- Use CV results for model comparison/model selection.
- Use final test results only as held-out confirmation.
...
- Explained variance tables/plots for the dimensionality reduction step.
...
# Number of PCA/SVD components used as ML features.
N_COMPONENTS = 50


# - "truncated_svd_sparse_safe" keeps the matrix sparse and is much more memory-friendly.
# - "pca_dense" performs real centered PCA but converts data to dense float32, which may be heavy.
DIM_REDUCTION_METHOD = "truncated_svd_sparse_safe"
# DIM_REDUCTION_METHOD = "pca_dense"
...
RESULTS_DIR = PROJECT_DIR / "results" / "all_models_level3_PCA"
FIGURES_DIR = RESULTS_DIR / "figures"

RESULTS_DIR.mkdir(parents=True, exist_ok=True)
FIGURES_DIR.mkdir(parents=True, exist_ok=True)

EXCEL_PATH = RESULTS_DIR / "all_models_level3_PCA_CV_results.xlsx"
METADATA_PATH = RESULTS_DIR / "all_models_level3_PCA_metadata.json"
SPLIT_PATH = RESULTS_DIR / "all_models_level3_PCA_split_indices.npz"

# Reuse the previous Level 1 LR split to keep Level 1, Level 3, and PCA-Level 3 comparisons consistent.
PREVIOUS_SPLIT_PATH = (
    PROJECT_DIR
    / "results"
    / "LR_level1"
...
def sparse_or_dense_to_dense_float32(X):
    """
    Convert input matrix to dense float32.

    This is only used when DIM_REDUCTION_METHOD == "pca_dense".
    It may use substantial RAM for large single-cell datasets.
    """
...
def make_dimensionality_reduction_step():
    """
    Create the dimensionality reduction part of the pipeline.

    Important:
    This step is fitted inside each CV fold, only on the training part of that fold.
    """
    if DIM_REDUCTION_METHOD == "truncated_svd_sparse_safe":
        return [
            (
                "scaler",
    ...
    if DIM_REDUCTION_METHOD == "pca_dense":
        return [
            (
                "to_dense_float32",
                FunctionTransformer(
                    sparse_or_dense_to_dense_float32,
                    accept_sparse=True,
                    validate=False,
                ),
            ),
    ...
    raise ValueError(
        "DIM_REDUCTION_METHOD must be either "
        "'truncated_svd_sparse_safe' or 'pca_dense'."
    )
...
def make_pipeline(classifier):
    """
    Full ML pipeline:
    scaling -> PCA/SVD-style dimensionality reduction -> classifier
    """
    steps = make_dimensionality_reduction_step()
...
def create_summary_table(fold_metrics_df, n_input_features, n_components, n_classes, n_dev, n_test):
    """
    Create one row per model by averaging only numeric fold-level columns.

    Important:
    Some columns in fold_metrics_df are descriptive text columns, for example
    dimensionality_reduction_method. Those must not be averaged.
    """
    excluded_cols = {
        "model_name",
        "algorithm",
    ...
    numeric_columns = fold_metrics_df.select_dtypes(include=[np.number]).columns.tolist()
    metric_columns = [
    ...
    for model_name in fold_metrics_df["model_name"].unique():
        ...
        row = {
            ...
            "feature_type": "PCA_components" if DIM_REDUCTION_METHOD == "pca_dense" else "TruncatedSVD_components",
            "dimensionality_reduction_method": DIM_REDUCTION_METHOD,
            "n_input_features_hvgs": n_input_features,
            "n_components_used_as_features": n_components,
            "n_classes": n_classes,
...
def save_presentation_summary_table(summary_df, output_path):
    ...
    table_df = table_df.rename(columns={
        ...
        "n_components_used_as_features": "Components",
        ...
        "explained_variance_sum_mean": "Explained Var.",
    })

    for col in table_df.columns:
        if col not in ["Model", "Class Weight", "Components"]:
    ...
    table.scale(1, 1.45)
    ax.set_title("Level 3 PCA/SVD Model Comparison: 5-Fold CV Summary", pad=12)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()


def save_explained_variance_plot(explained_variance_df, output_path):
    """
    Save a cumulative explained variance plot from the final models fitted on the development set.
    """
    ...
    for model_name in explained_variance_df["model_name"].unique():
        model_df = explained_variance_df[explained_variance_df["model_name"] == model_name]
        ax.plot(
            model_df["component_number"],
            model_df["cumulative_explained_variance_ratio"],
            marker="o",
            linewidth=1,
            markersize=3,
            label=model_name,
    ...
    ax.set_title("Cumulative explained variance of final fitted dimensionality reduction")
...
def get_explained_variance_sum(fitted_pipeline):
    reducer = fitted_pipeline.named_steps.get("dimensionality_reduction", None)
    if reducer is None:
        return np.nan
    if hasattr(reducer, "explained_variance_ratio_"):
        return float(np.sum(reducer.explained_variance_ratio_))
    return np.nan


def create_explained_variance_rows(fitted_pipeline, model_name, fit_scope):
    reducer = fitted_pipeline.named_steps.get("dimensionality_reduction", None)
    if reducer is None or not hasattr(reducer, "explained_variance_ratio_"):
        return []
    ...
    rows = []
    for i, (ratio, cumulative_ratio) in enumerate(zip(evr, cumulative), start=1):
        rows.append({
            "model_name": model_name,
            "fit_scope": fit_scope,
            "dimensionality_reduction_method": DIM_REDUCTION_METHOD,
...
N_CELLS = adata.n_obs
N_INPUT_FEATURES = adata.n_vars

if N_COMPONENTS >= N_INPUT_FEATURES:
    raise ValueError(
        f"N_COMPONENTS={N_COMPONENTS} must be smaller than the number of input features/HVGs "
        f"({N_INPUT_FEATURES})."
    )

print(f"\nNumber of cells: {N_CELLS}")
print(f"Number of input features/HVGs: {N_INPUT_FEATURES}")
print(f"Number of PCA/SVD components used as ML features: {N_COMPONENTS}")
print(f"Dimensionality reduction method: {DIM_REDUCTION_METHOD}")
...
# ============================================================
# 6. Reuse previous Level 1 development-test split
# ============================================================
...
if PREVIOUS_SPLIT_PATH.exists():
    ...
    raise FileNotFoundError(
        f"Previous Level 1 split file was not found:\n{PREVIOUS_SPLIT_PATH}\n"
...
print(f"Saved Level 3 PCA split indices to: {SPLIT_PATH}")
...
if dev_counts_check.min() < N_SPLITS:
    raise ValueError(
        f"At least one Level 3 class has fewer than {N_SPLITS} cells in the development set. "
        "5-fold stratified CV is not valid for this label level."
    )


# ============================================================
# 7. Label distribution and class info tables
# ============================================================
...
# ============================================================
# 8. Define candidate model pipelines
# ============================================================

models = {
    "LR_no_weight_PCA": {
        "model": make_pipeline(make_lr_model(class_weight=None)),
        "algorithm": "Logistic Regression",
        "class_weight": "None",
    },
    "LR_balanced_PCA": {
        "model": make_pipeline(make_lr_model(class_weight="balanced")),
        "algorithm": "Logistic Regression",
        "class_weight": "balanced",
    },
    "SVM_no_weight_PCA": {
        "model": make_pipeline(make_svm_model(class_weight=None)),
        "algorithm": "Linear SVM",
        "class_weight": "None",
    },
    "SVM_balanced_PCA": {
        "model": make_pipeline(make_svm_model(class_weight="balanced")),
        "algorithm": "Linear SVM",
        "class_weight": "balanced",
    },
    "RF_no_weight_PCA": {
        "model": make_pipeline(make_rf_model(class_weight=None)),
        "algorithm": "Random Forest",
        "class_weight": "None",
    },
    "RF_balanced_PCA": {
        "model": make_pipeline(make_rf_model(class_weight="balanced")),
        "algorithm": "Random Forest",
        "class_weight": "balanced",
    },
}
...
for model_name, model_info in models.items():
    ...
    row = {
        ...
        "n_input_features_hvgs": N_INPUT_FEATURES,
        "n_components": N_COMPONENTS,
        "reducer_class": reducer.__class__.__name__,
        "classifier_class": classifier.__class__.__name__,
    }

    for key, value in classifier.get_params().items():
        row[f"classifier__{key}"] = value

    for key, value in reducer.get_params().items():
        row[f"dimred__{key}"] = value
...
# ============================================================
# 9. Cross-validation on development set
# ============================================================
...
print("Important: dimensionality reduction is fitted inside each fold only on the fold training data.")
...
for model_name, model_info in models.items():
    ...
    for fold_number, (train_idx, val_idx) in enumerate(cv.split(X_dev, y_dev), start=1):
        ...
        explained_variance_sum = get_explained_variance_sum(model)
...
# ============================================================
# 10. Final held-out test evaluation for each candidate model
# ============================================================
...
for model_name, model_info in models.items():
    ...
    explained_variance_sum = get_explained_variance_sum(model)
    explained_variance_rows.extend(
        create_explained_variance_rows(
            fitted_pipeline=model,
            model_name=model_name,
            fit_scope="final_fit_on_development_set",
        )
    )
...
explained_variance_df = pd.DataFrame(explained_variance_rows)
...
presentation_summary = presentation_summary.rename(columns={
    ...
    "algorithm": "Algorithm",
    "class_weight": "Class Weight",
    "n_input_features_hvgs": "Input HVGs",
    "n_components_used_as_features": "Components",
    ...
    "explained_variance_sum_mean": "Explained Variance (CV mean)",
...
experiment_info = pd.DataFrame({
    ...
    "value": [
        ...
        (
            "TruncatedSVD is used as sparse-safe PCA-like dimensionality reduction."
            if DIM_REDUCTION_METHOD == "truncated_svd_sparse_safe"
            else "Dense centered PCA is used; this may require more RAM."
        ),
        N_CLASSES,
        ", ".join(CLASS_NAMES),
        TEST_SIZE,
        ...
        "StratifiedKFold on development set; dimensionality reduction fitted inside each fold",
        N_SPLITS,
        RANDOM_STATE,
        ", ".join(models.keys()),
        "f1_macro_mean",
        "Use CV results for model selection; use final_test_metrics only as held-out confirmation.",
        "Default method is sparse-friendly for 8GB RAM.",
    ],
})
...
metadata = {
    "experiment_name": "all_models_level3_PCA_CV",
    "date_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    "dataset_path": str(DATA_PATH),
    "label_column": LABEL_COLUMN,
    "n_cells_total": int(N_CELLS),
    "n_input_features_hvgs": int(N_INPUT_FEATURES),
    "n_components_used_as_features": int(N_COMPONENTS),
    "dimensionality_reduction_method": DIM_REDUCTION_METHOD,
    "dimensionality_reduction_note": (
        "TruncatedSVD is used as sparse-safe PCA-like dimensionality reduction."
        if DIM_REDUCTION_METHOD == "truncated_svd_sparse_safe"
    ...
    "cv_leakage_control": (
        "The scaler and dimensionality reduction step are fitted only on the training "
        "part of each CV fold because they are inside the sklearn Pipeline."
    ),
    "n_splits_cv": N_SPLITS,
    "random_state": RANDOM_STATE,
...
# ============================================================
# 13. Save one Excel workbook with all tables
# ============================================================
...
save_presentation_summary_table(
    ...
    output_path=FIGURES_DIR / "all_models_level3_PCA_presentation_summary_table.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="f1_macro_mean",
    title="Level 3 PCA/SVD Model Comparison: Macro F1 Mean Across 5-Fold CV",
    output_path=FIGURES_DIR / "all_models_level3_PCA_macro_f1_barplot.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="f1_weighted_mean",
    title="Level 3 PCA/SVD Model Comparison: Weighted F1 Mean Across 5-Fold CV",
    output_path=FIGURES_DIR / "all_models_level3_PCA_weighted_f1_barplot.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="balanced_accuracy_mean",
    title="Level 3 PCA/SVD Model Comparison: Balanced Accuracy Mean Across 5-Fold CV",
    output_path=FIGURES_DIR / "all_models_level3_PCA_balanced_accuracy_barplot.png",
)

if not explained_variance_df.empty:
    save_explained_variance_plot(
        explained_variance_df=explained_variance_df,
        output_path=FIGURES_DIR / "all_models_level3_PCA_cumulative_explained_variance.png",
    )
...
print("\nMemory note:")
print("- Default method is TruncatedSVD because it is sparse-friendly for 8GB RAM.")
print("- For real centered PCA, set DIM_REDUCTION_METHOD = 'pca_dense', but it may use much more memory.")

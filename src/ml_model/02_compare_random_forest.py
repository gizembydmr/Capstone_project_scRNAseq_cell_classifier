...
"""
Compare Random Forest models for PBMC68k cell type prediction.
...
RESULTS_DIR = PROJECT_DIR / "results" / "RF_level1"
FIGURES_DIR = RESULTS_DIR / "figures"

RESULTS_DIR.mkdir(parents=True, exist_ok=True)
FIGURES_DIR.mkdir(parents=True, exist_ok=True)

EXCEL_PATH = RESULTS_DIR / "RF_level1_CV_results.xlsx"
METADATA_PATH = RESULTS_DIR / "RF_level1_metadata.json"
...
def save_summary_table_image(summary_df, output_path):
    ...
    ax.set_title("Random Forest Level 1 CV Summary", pad=12)
...
print("\nStarting 5-fold stratified cross-validation for Random Forest...")
...
RF_no_weight_report = average_class_reports[
    average_class_reports["model_name"] == "RF_no_weight"
].reset_index(drop=True)

RF_balanced_report = average_class_reports[
    average_class_reports["model_name"] == "RF_balanced"
].reset_index(drop=True)

RF_no_weight_confusion_normalized = normalize_confusion_matrix_rows(
    confusion_matrices["RF_no_weight"]
)

RF_balanced_confusion_normalized = normalize_confusion_matrix_rows(
    confusion_matrices["RF_balanced"]
)
...
with pd.ExcelWriter(EXCEL_PATH, engine="openpyxl") as writer:
    ...
    confusion_matrices["RF_no_weight"].to_excel(
        writer,
        sheet_name="RF_no_weight_confusion",
    )

    confusion_matrices["RF_balanced"].to_excel(
        writer,
        sheet_name="RF_balanced_confusion",
    )

    RF_no_weight_confusion_normalized.to_excel(
        writer,
        sheet_name="RF_no_weight_conf_norm",
    )

    RF_balanced_confusion_normalized.to_excel(
        writer,
        sheet_name="RF_balanced_conf_norm",
    )

    RF_no_weight_report.to_excel(
        writer,
        sheet_name="RF_no_weight_report",
        index=False,
    )

    RF_balanced_report.to_excel(
        writer,
        sheet_name="RF_balanced_report",
        index=False,
    )
...
save_summary_table_image(
    ...
    output_path=FIGURES_DIR / "RF_level1_summary_table.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="f1_macro_mean",
    title="Random Forest Level 1: Macro F1 Comparison",
    output_path=FIGURES_DIR / "RF_level1_macro_f1_barplot.png",
)

save_metric_barplot(
    summary_df=summary_metrics,
    metric_col="balanced_accuracy_mean",
    title="Random Forest Level 1: Balanced Accuracy Comparison",
    output_path=FIGURES_DIR / "RF_level1_balanced_accuracy_barplot.png",
)

save_confusion_matrix_table(
    cm_df=confusion_matrices["RF_no_weight"],
    title="RF no weight - summed CV confusion matrix",
    output_path=FIGURES_DIR / "RF_level1_no_weight_confusion_table.png",
)

save_confusion_matrix_table(
    cm_df=confusion_matrices["RF_balanced"],
    title="RF balanced - summed CV confusion matrix",
    output_path=FIGURES_DIR / "RF_level1_balanced_confusion_table.png",
)

save_normalized_confusion_matrix_table(
    cm_df=confusion_matrices["RF_no_weight"],
    title="RF no weight - normalized summed CV confusion matrix (%)",
    output_path=FIGURES_DIR / "RF_level1_no_weight_confusion_table_normalized.png",
)

save_normalized_confusion_matrix_table(
    cm_df=confusion_matrices["RF_balanced"],
    title="RF balanced - normalized summed CV confusion matrix (%)",
    output_path=FIGURES_DIR / "RF_level1_balanced_confusion_table_normalized.png",
)
...
print("\nRandom Forest cross-validation finished successfully.")

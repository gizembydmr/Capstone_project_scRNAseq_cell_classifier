# 10_compare_unassigned_thresholds_level3.py

"""
Compare confidence thresholds for adding an Unassigned label to the final
Level 3 Logistic Regression model.

This script trains the same LR model on the 80% development set and evaluates
...
RESULTS_DIR = PROJECT_DIR / "results" / "unassigned_thresholds_level3"
FIGURES_DIR = RESULTS_DIR / "figures"

RESULTS_DIR.mkdir(parents=True, exist_ok=True)
FIGURES_DIR.mkdir(parents=True, exist_ok=True)

EXCEL_PATH = RESULTS_DIR / "LR_level3_unassigned_threshold_comparison.xlsx"
METADATA_PATH = RESULTS_DIR / "LR_level3_unassigned_threshold_comparison_metadata.json"
...
def save_threshold_tradeoff_plot(threshold_summary_df, output_path):
    ...
    plt.title(
        "Confidence Threshold Trade-off for Level 3 LR Model",
        fontsize=16,
        fontweight="bold",
        pad=16,
    )
...
def save_threshold_summary_table(presentation_summary_df, output_path, selected_threshold=None):
    ...
    ax.set_title(
        "Level 3 LR Confidence Threshold Comparison",
        fontsize=17,
        fontweight="bold",
        pad=18,
    )
    ...
    ax.text(
        0.5,
        -0.08,
        "The 0.50 threshold is highlighted as a conservative MVP choice; threshold choice should consider both coverage and reliability.",
...
if not SPLIT_PATH.exists():
    raise FileNotFoundError(
        ...
        "Please run the Level 1 LR comparison script first."
    )
...
baseline_metrics_df = pd.DataFrame([{
    "model_name": "LR_level3_no_weight_final_without_unassigned",
...
for threshold in THRESHOLDS:
    ...
    save_normalized_confusion_matrix_light_heatmap(
        cm_df,
        title=f"Level 3 LR with Unassigned threshold {threshold:.2f}",
        output_path=FIGURES_DIR / f"LR_level3_unassigned_threshold_{sheet_suffix}_heatmap.png",
    )
...
summary_table_path = FIGURES_DIR / "LR_level3_unassigned_threshold_summary_table.png"
tradeoff_plot_path = FIGURES_DIR / "LR_level3_unassigned_threshold_tradeoff_plot.png"
coverage_plot_path = FIGURES_DIR / "LR_level3_unassigned_threshold_assigned_unassigned_plot.png"
...
metadata = {
    "script_name": "10_compare_unassigned_thresholds_level3.py",
